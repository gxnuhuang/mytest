...
#include<id3tag.h>
#if 0
char *filename="/home/gxnuhuang/madplay_code/cn/002.mp3";
	  static struct hinfo {
	    char  *id;
	    char  *label;
	  } ;
int main(int argc, char *argv[])
{
	...
	struct id3_tag *pid3_tag=NULL;
	struct id3_frame *pid3_frame=NULL;
	int i;
	int fd=0;
...
struct hinfo info[] = {
	...
	//file=id3_file_open(filename, 1);
	file = id3_file_fdopen(fd, ID3_FILE_MODE_READONLY);
	if(NULL == file){
	printf("open fail\n");
	}
	pid3_tag=id3_file_tag(file);
	if(NULL == pid3_tag){
	printf("tag fail\n");
	}
	debug_msg("refcount=%d nframes=%d paddedsize=%d\n",pid3_tag->refcount,pid3_tag->nframes,pid3_tag->paddedsize);
	  for (i = 0; i < sizeof(info) / sizeof(info[0]); ++i) {
	    union id3_field const *field;
	    unsigned int nstrings, j;

	    pid3_frame = id3_tag_findframe(pid3_tag, info[i].id, 0);
		printf("hzq:find framfs1\n");
	    if (pid3_frame == 0){
		printf("[%d] fail\n",i);
		}else{
		printf("[%d] yes\n",i);
  ...
  *******************************************************************************/   
  ...
  printf("ID3V2_size:%d   bytes/n",ID3V2_size);   
  iFrameStart=--ID3V2_size;   
  }   
  else   
  {   
  bHasID3V2=0;   
  printf("/nThere   is   no   ID3V2*");   
  }   
  ...
  {   
  case   0:   
  printf("MPEG   2.5   层   %d,   %dHz   %s/n",4-LayerDescript,   
  ...
  printf("CBR:->   %dKbit     %d帧/n",Bitrate,FrameCount);   
  printf("长度:%d   秒",mp3Duration);   
  break;   
  case   2:   
  printf("MPEG   2.0   层   %d,   %dHz   %s/n",4-LayerDescript,   
  ...
  break;   
  case   3:   
  printf("MPEG   1.0   层   %d,   %dHz   %s/n",4-LayerDescript,   
  ...
  printf("/nThis   a   Free   Rate   MP3   File!/n");   
  ...
  {   
  printf("/n*/nID3V1_size:%d   bytes/n",ID3V1_size   );   
